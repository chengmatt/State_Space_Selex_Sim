    ...
    if(sum(om_application %in% c(om_scenarios$OM_Scenarios[n_om])) >= 1) {
      ...
      input <- prepare_EM_input(years = years,
                                ...
                                F_Slx_Model_Input = matrix(
                                  c(rep("logistic", 50), rep("exp_logistic", 50)),
                                  nrow = 50, ncol = 2
      ...
      plot(model$model_fxn$rep$F_Slx[1,,1,1])
      plot(model$model_fxn$rep$F_Slx[1,,1,2])
      plot(model$model_fxn$rep$F_Slx[1,,2,1], ylim = c(0,1))
      plot(model$model_fxn$rep$F_Slx[1,,2,2], ylim = c(0,1))
...
for(i in 1:50) {
  if(i == 1)plot(input$data$obs_fish_age_comps[1,,2,2], type = "l")
  else lines(input$data$obs_fish_age_comps[i,,2,2], type = "l")
}
