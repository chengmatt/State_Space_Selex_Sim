# Purpose: To run EMs with 1 Fleet and time-blocking as a sensitivity test
# Note that I am only doing this for the fast fleet structure changes, to compare
# how well AIC might or might not pick an adequate time period to block 
...
for(n_om in 1:n_OM_scen) {
  ...
  for(n_em in 1:n_EM_scen) {
    ...
    # Specify number of yeras
    years <- 1:em_scenarios$n_years[n_em]
    ...
    blk_time = em_scenarios$Blk_Time[n_em]
    
    # set up blocking structure
    F_Slx_Blocks_Input <- matrix(c(rep(0, blk_time), rep(1, length(years) - blk_time)), 
                                 nrow = length(years), ncol = n_fleets)
    
    # set up matrix for specifying selex form at a given block
    fish_selex_opt <- matrix(c(rep(fish_selex_opt[1], blk_time), 
