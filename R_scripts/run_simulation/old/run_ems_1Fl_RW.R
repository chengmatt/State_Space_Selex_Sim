...
# Pre-process EM ----------------------------------------------------------
    ...
    for(i in 1:50) {
      if(i == 1) plot(model$model_fxn$rep$F_Slx[1,,,1], ylim = c(0,1))
    ...
    write.csv(AIC_df, here(em_path_res, "AIC_Results.csv"))    write.csv(params, here(em_path_res, "Param_Results.csv"))
