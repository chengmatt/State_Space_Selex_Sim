...
Type objective_function<Type>::operator() ()
{
  ...
  for(int fi = 0; fi < n_fish_indices; fi++) {
    for(int y = 0; y < n_years; y++) { 
      ...
      Type tmp_q_fish = Type(0) + (Type(0.5) - Type(0))/ (1 + exp(-logit_q_fish(fi)));
      // Scale by catchability
      pred_fish_indices(y, fi) *= tmp_q_fish; 
    } // y loop
  } // fi loop
  ...
  for(int si = 0; si < n_srv_indices; si++) {
    for(int y = 0; y < n_years; y++) {
      ...
      // Inverse logit
      Type tmp_q_srv = Type(0) + (Type(0.5) - Type(0))/ (1 + exp(-logit_q_srv(si))); 
      pred_srv_indices(y, si) *= tmp_q_srv; // Scale by catchability
    } // y loop
  } // si loop  
