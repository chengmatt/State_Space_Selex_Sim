# Purpose: To run EMs that aren't self-tests (full-factorial design here) for Recruitment pulse scenario
...
ncores <- detectCores() 
cl <- makeCluster(ncores - 2, type = "SOCK")
registerDoSNOW(cl)
...
om_scenarios <- readxl::read_excel(here('input', "OM_EM_Scenarios.xlsx"), sheet = "OM_RecPulse")
...
for(n_om in 1:n_OM_scen) {
  ...
  for(n_em in 1:n_EM_scen) {
    ...
    tryCatch(expr = sim_models <- foreach(sim = 1:n_sims,
                          .packages = c("TMB", "here", "tidyverse"), .export = ls(globalenv())) %dopar% {
        # for(sim in 1:n_sims) {
        compile_tmb(wd = here("src"), cpp = "EM.cpp")
        ...
        input <- prepare_EM_input(years = years,
                                  ...
                                  rec_model = "BH", 
                                  F_Slx_Model_Input = as.matrix(c(rep("gamma", 25), rep("logistic", 25)),
        ...
        all_obj_list
      }, error = function(e){e}) # end foreach loop
