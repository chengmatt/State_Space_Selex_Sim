...
# Catch related quantities (Fishery) ------------------------------------------------
  ...
  # Need to munge to get into matrix format
    ...
    mutate(Year = parse_number(as.character(Year)),
      ...
      filter(Year >= Fish_Start_yr,
             Sim == n_sims)
      
    # Get catch and sum across
    catch_vec <- tapply(catch_df$Catch, catch_df$Year, FUN = sum)
    
  # Now stick that into the matrix and put it into the input list - add some lognormal
  # noise to this
    
    if(catch_error == TRUE) {
      agg_catch[, 1] <- catch_vec * exp(rnorm(length(catch_vec), 0, log((Catch_CV_Val^2) +1)))
    } else{
      agg_catch[, 1] <- catch_vec
    } # else = no error
  ...
  fishcomps_df <- tapply( fish_comps$Count,fish_comps$Year, FUN=function(x) { x/sum(x) })
...
# Indices and related quantities -----------------------------------------------------------------
  ...
  # Just makes it easier for us to use in WHAM
  ...
  for(y in 1:ncol(index_paa)) {
      ...
      # stick our fihs comps list from tapply into here
      index_paa[1, y, ] <- fishcomps_df[[y]]
  ...
  # Get proportions (group by summarize)
  survcomps_df <- tapply(surv_comps$Count, surv_comps$Year, FUN=function(x) { x/sum(x) })
  ...
  for(y in 1:length(year_vec)) {  # length year vec - because needs to correspond to # of unique survey years
      ...
      index_paa[2, y + (length(Fish_Start_yr:Surv_Start_yr)-1) ,] <- survcomps_df[[y]]
...
# Biologicals (Weight at age and Maturity at age) --------------------------------------
  ...
  maturity[,] <- mat_at_age[min(input$years):max(input$years),,n_sims]
  # Put into our list
  ...
  for(i in 1:nwaa) {
    ...
    waa[i,,] <- wt_at_age[min(input$years):max(input$years),,n_sims]
