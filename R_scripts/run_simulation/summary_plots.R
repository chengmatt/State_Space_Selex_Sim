...
plot_order <- c("2Fl_LL", "2Fl_LGam",  "1Fl_L_TI", "1Fl_Gam_TI", 
                "1Fl_LL_Blk", "1Fl_LGam_Blk", "1Fl_L_RW_1.25", "1Fl_Gam_RW_2.0")
...
# One fleet time block models (Terminal Year)
(onefleet_blk_term_plot <- ggplot(onefleet_blk_aic %>% filter(time_comp == "Terminal"),
                             aes(x = OM_Scenario, y = EM_Scenario, fill = n_minAIC,
                                 label = round(n_minAIC, 2))) +
    geom_tile(alpha = 0.65) +
    geom_text(size = 4) +
...
# One fleet time block models (TrxE)
(onefleet_blk_TrxE_plot <- ggplot(onefleet_blk_aic %>% filter(time_comp == "Fleet Trans End"),
                                  aes(x = OM_Scenario, y = EM_Scenario, fill = n_minAIC,
                                      label = round(n_minAIC, 2))) +
    geom_tile(alpha = 0.65) +
    geom_text(size = 4) +
...
# Filter to relevant components for parameters
om_scenario_params <- param_df %>% filter(time_comp %in% c("Terminal", "Fleet Trans End"),
                                          str_detect(OM_Scenario, "Fast"),
...
term_biom_ssb_blk <- ts_re_df %>% 
  filter(
    ...
    par_name %in% c("Spawning Stock Biomass"),
    str_detect(EM_Scenario, "Blk"),
    time_comp %in% c("Terminal", "Fleet Trans End"),
    str_detect(OM_Scenario, "Fast")) %>% 
...
pdf(here(dir_out, "Param_Sum_BlkModels.pdf"), width = 35, height = 15)
  ...
  print(
    ggplot(pt_rg_re_blk %>% 
             filter(str_detect(OM_Scenario, "Low")), 
             aes(x = factor(EM_Scenario), y = median, 
                 ymin = lwr_95, ymax = upr_95, color = time_comp)) +
      geom_pointrange(position = position_dodge2(width = 1), 
                      size = 1, linewidth = 1, alpha = 1) +
      geom_hline(aes(yintercept = 0), col = "black", lty = 2, size = 0.5, alpha = 1) +
...
pdf(here(dir_out, "TS_Sum.pdf"), width = 18, height = 15)
...
pdf(here(dir_out, "TS_Sum_RW.pdf"), width = 25, height = 15)
...
for(i in 1:length(ts_pars)) {
  ...
  ts_re_om <- ts_re_df %>% filter(time_comp %in% c("Terminal", "Fleet Trans End"),
    ...
    # Now loop through each time series component and print the plot out
    print(
      ggplot(ts_re_om %>% filter(Dat_Qual == "Low", par_name == ts_pars[i]), 
             aes(x = year, y = median, fill = time_comp, color = time_comp))  +
...
pdf(here(dir_out, "NAA_Summary.pdf"), width = 40, height = 25)
for(i in 1:length(unique_oms)) {
  ...
  print(
    ggplot(NAA_df_re_plot %>%
             filter(Sex == "Female", Age %in% c(seq(3, 27, 3), 27:30)),
  ...
  print(
    ggplot(NAA_df_re_plot %>%
             filter(Sex == "Male", Age %in% c(seq(3, 30, 3))),
           aes(x = Year, y = Median_RE, fill = time_comp)) +
      geom_line(linewidth = 2, alpha = 1, aes(color = time_comp)) +
      geom_ribbon(aes(ymin = Lwr_95, ymax = Upr_95), alpha = 0.3) +
      geom_hline(aes(yintercept = 0), col = "black",
...
for(i in 1:length(unique_oms)) {
  ...
  print(
    ggplot() +
      ...
      labs(x = "Exploitation Rate", y = "SPR", color = "", title = paste(unique_oms[i], "Terminal")) +
      theme_matt() +
      theme(legend.position = "top", 
            title = element_text(size = 20))
  )
  ...
  print(
    ggplot() +
      # SPR lines - EM (Fleet Trans End)
      geom_line(em_plot_df %>% filter(time_comp == "Fleet Trans End"),
                mapping = aes(x = trial_F, y = SPR, color = "Estimated"),
                lwd = 1, alpha = 0.75) +
      # SPR lines - OM
      geom_line(om_plot_df %>% filter(time_comp == "Fleet Trans End"), 
                mapping = aes(x = trial_F, y = SPR, color = "True"), 
                lwd = 1, alpha = 0.75, lty = 2) +
      # F40 lines - EM
      geom_vline(em_f40 %>% filter(time_comp == "Fleet Trans End"), 
                 mapping = aes(xintercept = f40_em, color = "Estimated"),
                 lwd = 1, lty = 1, alpha = 0.75) +
      # F40 lines - OM
      geom_vline(om_f40 %>% filter(time_comp == "Fleet Trans End"),
                 mapping = aes(xintercept = f40_om, color = "True"),
                 lwd = 1, lty = 2, alpha = 0.75) +
      # Geom text for relative error
      geom_text(em_f40 %>% filter(time_comp == "Fleet Trans End"),
                mapping = aes(x = 0.4, y = 0.85, label = paste("RE:", round(re, 3) * 100)))+
      scale_color_manual(values = c("blue", "black"), label = c("Estimated", "True")) +
      facet_grid(Mat_Opt~EM_Scenario) +
      labs(x = "Exploitation Rate", y = "SPR", color = "", title = paste(unique_oms[i], "Fleet Trans End")) +
      theme_matt() +
      theme(legend.position = "top", 
            title = element_text(size = 20))
  )
  ...
  print(
    ggplot() +
      # SPR lines - EM (Fleet Intersect)
      geom_line(em_plot_df %>% filter(time_comp == "Fleet Intersect"),
                mapping = aes(x = trial_F, y = SPR, color = "Estimated"),
                lwd = 1, alpha = 0.75) +
      # SPR lines - OM
      geom_line(om_plot_df %>% filter(time_comp == "Fleet Intersect"), 
                mapping = aes(x = trial_F, y = SPR, color = "True"), 
                lwd = 1, alpha = 0.75, lty = 2) +
      # F40 lines - EM
      geom_vline(em_f40 %>% filter(time_comp == "Fleet Intersect"), 
                 mapping = aes(xintercept = f40_em, color = "Estimated"),
                 lwd = 1, lty = 1, alpha = 0.75) +
      # F40 lines - OM
      geom_vline(om_f40 %>% filter(time_comp == "Fleet Intersect"),
                 mapping = aes(xintercept = f40_om, color = "True"),
                 lwd = 1, lty = 2, alpha = 0.75) +
      # Geom text for relative error
      geom_text(em_f40 %>% filter(time_comp == "Fleet Intersect"),
                mapping = aes(x = 0.4, y = 0.85, label = paste("RE:", round(re, 3) * 100)))+
      scale_color_manual(values = c("blue", "black"), label = c("Estimated", "True")) +
      facet_grid(Mat_Opt~EM_Scenario) +
      labs(x = "Exploitation Rate", y = "SPR", color = "", title = paste(unique_oms[i], 
                                                                         "Fleet Intersect")) +
      theme_matt() +
      theme(legend.position = "top", 
            title = element_text(size = 20))
...
# Get SPR differences Block Models (Maturity Sensitivity) ------------------------------
# Only plotting terminal years (b/c the block models only do that)
pdf(file = here(dir_out, "SPR_PopSelex_MatSense_BlkModels.pdf"), width = 75, height = 10)
# Now plot this out!
for(i in 1:length(unique_oms)) {
